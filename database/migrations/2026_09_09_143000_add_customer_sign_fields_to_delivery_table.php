...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('delivery', function (Blueprint $table) {
            if (!Schema::hasColumn('delivery', 'sign')) {
                $table->string('sign')->nullable()->after('code');
            }
            if (!Schema::hasColumn('delivery', 'sign_token')) {
                $table->string('sign_token', 64)->nullable()->unique()->after('sign');
            }
            if (!Schema::hasColumn('delivery', 'customer_signature')) {
                $table->string('customer_signature')->nullable()->after('sign_token');
            }
            if (!Schema::hasColumn('delivery', 'customer_signer_name')) {
                $table->string('customer_signer_name')->nullable()->after('customer_signature');
            }
            if (!Schema::hasColumn('delivery', 'customer_signer_position')) {
                $table->string('customer_signer_position')->nullable()->after('customer_signer_name');
            }
            if (!Schema::hasColumn('delivery', 'customer_signed_stamp')) {
                $table->string('customer_signed_stamp')->nullable()->after('customer_signer_position');
            }
            if (!Schema::hasColumn('delivery', 'customer_signed_at')) {
                $table->dateTime('customer_signed_at')->nullable()->after('customer_signed_stamp');
            }
            if (!Schema::hasColumn('delivery', 'customer_ip')) {
