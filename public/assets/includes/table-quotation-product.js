$(function () {
    var dt_table_product_quotation = $(".datatable-product-quotation");
    var Url = "/db/product/quotation/detail/";
    var path = window.location.pathname;
    var id = path.substring(path.lastIndexOf("/") + 1);

    // console.log("ID:", id); // Output: ID: 2

    if (dt_table_product_quotation.length) {
        $('[data-toggle="tooltip"]').tooltip();
        var dt_product = dt_table_product_quotation.DataTable({
            ajax: {
                type: "GET",
                url: Url + id,
                headers: {
