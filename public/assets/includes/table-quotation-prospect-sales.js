$(function () {
    var dt_table = $(".datatable-quotation-prospect-sales");
    var Url = "/db/quotation/prospect/sales";

    if (dt_table.length) {
        var dt_prospect_sales = dt_table.DataTable({
            ajax: {
                type: "GET",
                url: Url,
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full) {
                        ...
                        return `<span class="d-inline-block text-truncate text-muted small" style="max-width: 240px;" data-bs-toggle="tooltip" title="${escaped}">${escaped}</span>`;
                    },
                },
                {
            ...
            order: [[4, "asc"]],
            dom: '<"card-header flex-column flex-md-row d-flex justify-content-between align-items-center py-3"<"head-label hl-qp-sales"><"d-flex align-items-center gap-2"f>>t<"card-footer d-flex flex-column flex-md-row justify-content-between align-items-center py-3"<"small text-muted"i><"pagination-wrapper"p>>',
        ...
        $("div.hl-qp-sales").html(
            '<div class="d-flex align-items-center gap-2">' +
                '<div class="avatar avatar-sm bg-label-primary rounded p-1">' +
                    '<i class="mdi mdi-file-document-multiple-outline fs-4"></i>' +
                '</div>' +
                '<div>' +
                    '<h6 class="fw-bold mb-0 text-heading">Daftar Quotation Prospect Saya</h6>' +
                    '<small class="text-muted">Kelola penawaran aktif hasil prospek yang ditugaskan kepada Anda</small>' +
                '</div>' +
            '</div>'
        );
