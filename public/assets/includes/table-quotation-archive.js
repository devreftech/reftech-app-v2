$(function () {
    var dt_table = $(".datatable-quotation-archive");
    var Url = "/db/quotation/archive";
    ...
    if (dt_table.length) {
        ...
        dt_table.find("thead tr:eq(1) th").each(function (i) {
            ...
            $("input", this).on("keyup change", function () {
                if (dt_archive.column(i).search() !== this.value) {
                    dt_archive.column(i).search(this.value).draw();
                }
            });
        });

        var dt_archive = dt_table.DataTable({
            ajax: {
                type: "GET",
                url: Url,
                ...
                },
                {
                    ...
                    render: function (data, type, full) {
                        ...
                        return '<span class="badge rounded-pill ' + s.class + ' cursor-pointer"' +
                            ...
                            ' data-bs-custom-class="' + (s.colorTip || 'tooltip-secondary') + '" title="' + escapedTip + '">' +
                            label + "</span>";
                    },
                },
            ],
