$(function () {
    function initSellingDataTable(tableSelector, taxValue) {
        var $table = $(tableSelector);
        if (!$table.length) return null;

        function getUrl() {
            var year = $('#filter-year-selling').val() || 'all';
            return '/db/selling-contract?year=' + year + '&tax=' + taxValue;
        }
    ...
    var dtPpn = initSellingDataTable(".datatable-selling-contract-ppn", "ppn");
    var dtNonPpn = initSellingDataTable(".datatable-selling-contract-non-ppn", "non-ppn");

    $('#filter-year-selling').on('change', function () {
        var year = $(this).val() || 'all';
        if (dtPpn) dtPpn.ajax.url('/db/selling-contract?year=' + year + '&tax=ppn').load();
        if (dtNonPpn) dtNonPpn.ajax.url('/db/selling-contract?year=' + year + '&tax=non-ppn').load();
    });
