$(function () {
    var dt_table_overview_crm = $(".datatable-overview-crm");
    var Url = "/db/overview/crm/";
    var path = window.location.pathname;
    var segments = path.split("/").filter(function (s) { return s.length > 0; });

    var sales = segments[segments.length - 2];
    var dateRep = segments[segments.length - 1];
    var targetUrl = dt_table_overview_crm.data("url") || (Url + sales + "/" + dateRep);
    ...
    function formatMinimalistList(d) {
        ...
        if (history.length === 0) {
            itemsHtml = '<div class="text-center text-muted py-3 fst-italic">Belum ada riwayat interaksi CRM detail.</div>';
        } else {
            history.forEach(function (item) {
                ...
                var iconName = isResponded ? "mdi-account-check" : "mdi-account-cancel";
                        ...
                        '<div class="flex-grow-1 min-w-0">' +
                            '<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-1">' +
                                '<div class="d-flex align-items-center gap-2 flex-wrap">' +
                                    ...
                                    '<span class="badge bg-label-primary rounded-pill fs-tiny">CRM Touch</span>' +
            ...
            '<div class="p-3 my-2 rounded-3 bg-light border shadow-xs">' +
                '<div class="d-flex align-items-center justify-content-between mb-3 pb-2 border-bottom flex-wrap gap-2">' +
                    '<div class="d-flex align-items-center gap-2">' +
                        ...
                        '</div>' +
                        '<div>' +
                            '<span class="text-muted fs-tiny fw-semibold text-uppercase d-block">Riwayat Interaksi CRM (Minimalist List)</span>' +
                            '<h6 class="mb-0 fw-bold text-dark">' + (d.company || "-") + '</h6>' +
                        '</div>' +
                    '</div>' +
                    '<div class="d-flex align-items-center gap-2">' +
                        '<span class="badge bg-primary rounded-pill px-3 py-1 fw-semibold"><i class="mdi mdi-account-multiple me-1"></i>' + totalCount + ' Total CRM Touch</span>' +
    ...
    if (dt_table_overview_crm.length) {
        var dt_crm = dt_table_overview_crm.DataTable({
            ajax: {
                type: "GET",
                url: targetUrl,
                headers: {
                ...
                },
                {
                    ...
                    render: function (data, type, full) {
                        if (type === "display") {
                            ...
                            return '<span class="badge ' + color + ' rounded-pill fw-semibold px-2 py-1"><i class="mdi mdi-account-multiple-outline me-1"></i>' + count + 'x CRM</span>';
                        }
                        return data || 1;
                    },
                },
            ...
            language: {
                ...
                emptyTable: "Tidak ada data CRM",
            },
        });

        // Add event listener for opening and closing details
        dt_table_overview_crm.find("tbody").on("click", "td.dt-control", function () {
            var tr = $(this).closest("tr");
            var row = dt_crm.row(tr);
