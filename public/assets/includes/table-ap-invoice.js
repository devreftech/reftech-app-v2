$(function () {
    var dt_table_sales_invoice_ap = $(".datatable-sales-invoice-ap");
    var Url = "/db/payable/invoice";

    if (dt_table_sales_invoice_ap.length) {
        $('[data-toggle="tooltip"]').tooltip();
        // Setup - add a text input to each footer cell
        $(".datatable-sales-invoice-ap thead tr")
            .clone(true)
            .appendTo(".datatable-sales-invoice-ap thead");
        $(".datatable-sales-invoice-ap thead tr:eq(1) th").each(function (i) {
        ...
        var dt_filter = dt_table_sales_invoice_ap.DataTable({
            ajax: {
                type: "GET",
                url: Url,
                headers: {
    ...
    dt_table_sales_invoice_ap.on("draw", function () {
        $('[data-toggle="tooltip"]').tooltip();
    });
});
