        ...
        var dt_prospect = dt_table_prospect.DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full) {
                        if (type === "display") {
                            if (full.sales || full.name) {
                                var salesName = full.sales || full.name;
                                var initial = salesName.charAt(0).toUpperCase();
                                var avatarHtml = (full.sales_image || full.image)
                                    ? `<img src="/${full.sales_image || full.image}" class="rounded-circle shadow-xs" width="30" height="30" style="object-fit:cover;" onerror="this.outerHTML='<span class=\\'avatar-initial rounded-circle bg-label-primary small\\'>${initial}</span>'">`
            ...
            order: [[2, "desc"]],
            dom: '<"card-header flex-column flex-md-row d-flex justify-content-between align-items-center py-3"<"head-label hl-supp"><"d-flex align-items-center gap-2"f>>t<"card-footer d-flex flex-column flex-md-row justify-content-between align-items-center py-3"<"small text-muted"i><"pagination-wrapper"p>>',
            displayLength: 10,
