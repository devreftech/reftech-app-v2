$(function () {
    ...
    var targetUrl = dt_table_overview_po_prospect.data("url") || (Url + sales + "/" + dateRep);

    if (dt_table_overview_po_prospect.length) {
        var dt_po_prospect = dt_table_overview_po_prospect.DataTable({
            ajax: {
                type: "GET",
                url: targetUrl,
                headers: {
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full) {
                        if (type === "display") {
                            var $dataId = full["id"];
                            var detailRoute = typeof route === "function" ? route("quotation.show", $dataId) : "/quotation/" + $dataId;
                            return '<a class="text-dark fw-semibold" href="' + detailRoute + '">' + (data || "-") + "</a>";
                        }
                        return data || "-";
                    },
                },
            ...
            language: {
                ...
                emptyTable: "Tidak ada data PO prospect",
            },
        });
    }
});
