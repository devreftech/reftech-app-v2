...
@section('title', 'Edit Estimasi / RAB: ' . $rab->no_rab)
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1">
                <h4 class="fw-bold mb-0 text-dark">Edit Estimasi / RAB: {{ $rab->no_rab }}</h4>
                <span class="autosave-pill" id="autoSaveIndicator">
                    <i class="mdi mdi-check-circle" style="font-size: 14px;"></i> Sesi Aktif
                </span>
            </div>
            <p class="text-muted mb-0 small">Perbarui data teknis lapangan, ganti vendor HPP, margin laba, atau status RAB.</p>
        </div>

        <div class="d-flex gap-2">
            <a href="{{ route('piping-rab.show', $rab->id) }}" class="btn btn-outline-secondary btn-sm">
                <i class="mdi mdi-arrow-left me-1"></i> Kembali ke Detail
            </a>
        </div>
    </div>

    <form id="formRab" action="{{ route('piping-rab.update', $rab->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="rab-card mb-4">
            <div class="section-header-clean d-flex align-items-center justify-content-between">
                ...
                <span class="badge bg-label-primary fw-bold px-3 py-2">{{ $rab->no_rab }}</span>
            </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        <label class="form-label small fw-bold text-primary required">
                            <i class="mdi mdi-account-tie me-1"></i> 1. Sales Person
                        </label>
                        <select name="id_sales" id="selectSales" class="form-select form-select-clean fw-semibold" required onchange="onSalesChanged(this.value)">
                            ...
                            @foreach($salesList as $s)
                                <option value="{{ $s->id }}" {{ $rab->id_sales == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                            @endforeach
                        </select>
                    </div>

                    <!-- 2. Customer / Perusahaan (Searchable) -->
                    <div class="col-md-4">
                        <label class="form-label small fw-bold text-primary required">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="id_pic" id="selectPic" class="form-select form-select-clean">
                            ...
                            @foreach($pics as $p)
                                <option value="{{ $p->id }}" {{ $rab->id_pic == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="date" name="rab_date" id="inputRabDate" class="form-control form-control-clean" value="{{ $rab->rab_date ? $rab->rab_date->format('Y-m-d') : date('Y-m-d') }}" required>
                    </div>

                    <!-- 5. Nama Proyek -->
                    <div class="col-md-5">
                        <label class="form-label small fw-semibold text-muted required">Nama Pekerjaan / Proyek</label>
                        <input type="text" name="project_name" id="inputProjectName" class="form-control form-control-clean" value="{{ $rab->project_name }}" required>
                    </div>

                    <!-- 6. Lokasi Plant -->
                    <div class="col-md-4">
                        <label class="form-label small fw-semibold text-muted">Lokasi / Area Plant</label>
                        <input type="text" name="location_plant" id="inputLocationPlant" class="form-control form-control-clean" value="{{ $rab->location_plant }}">
                    </div>

                    <!-- 7. Status RAB -->
                    <div class="col-md-3">
                        <label class="form-label small fw-semibold text-muted required">Status RAB</label>
                        <select name="status" class="form-select form-select-clean fw-bold" required>
                            <option value="Draft" {{ $rab->status === 'Draft' ? 'selected' : '' }}>Draft</option>
                            <option value="Reviewed" {{ $rab->status === 'Reviewed' ? 'selected' : '' }}>Reviewed</option>
                            <option value="Approved" {{ $rab->status === 'Approved' ? 'selected' : '' }}>Approved</option>
                            <option value="Converted" {{ $rab->status === 'Converted' ? 'selected' : '' }}>Converted</option>
                        </select>
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="notes" id="inputNotes" class="form-control form-control-clean" rows="1">{{ $rab->notes }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="sectionsContainer">
            <!-- Populated dynamically via JS with existing data -->
        </div>
        ...
        <div class="sticky-summary-bar p-3 mb-4">
            <div class="row align-items-center gx-3 gy-2 m-0 p-0">
                ...
                <div class="col-lg-4 text-end d-flex justify-content-end align-items-center gap-2">
                    <a href="{{ route('piping-rab.show', $rab->id) }}" class="btn btn-outline-secondary btn-sm">Batal</a>
                    <button type="submit" class="btn btn-primary px-4 py-2 fw-semibold">
                        <i class="mdi mdi-content-save-outline me-1"></i> Update Estimasi RAB
                    </button>
                </div>
            </div>
        </div>
...
<script>
    ...
    const existingSections = @json($rab->sections);
    const initialClientId = @json($rab->id_client);
    const initialPicId = @json($rab->id_pic);
    const initialSalesId = @json($rab->id_sales);
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        setInterval(function () {
            fetch('/piping-materials/search-api?category=pipe&limit=1')
                .then(() => {
                    ...
                    if (indicator) {
                        indicator.innerHTML = '<i class="mdi mdi-check-circle" style="font-size: 14px;"></i> Sesi Aktif';
                    }
                })
                .catch(() => console.log('Keep-alive ping error'));
        }, 300000);

        // Load clients of current sales
        if (initialSalesId) {
            onSalesChanged(initialSalesId, initialClientId, initialPicId);
        }
        ...
        // Populate Existing Sections & Items
        if (existingSections && existingSections.length > 0) {
            existingSections.forEach(sec => {
                const sIndex = addSection(sec.section_name);
                const itemsContainer = document.getElementById(`items_container_${sIndex}`);
                itemsContainer.innerHTML = '';

                if (sec.items && sec.items.length > 0) {
                    sec.items.forEach(item => {
                        addItem(sIndex, item.item_type || 'material', item);
                    });
                } else {
            ...
            .then(data => {
                ...
                if (data.clients && data.clients.length > 0) {
                    data.clients.forEach(c => {
                        const isSel = (preselectedClientId && preselectedClientId == c.id) || (initialClientId && initialClientId == c.id) ? 'selected' : '';
                ...
                const chosenClientId = preselectedClientId || selectClient.val();
                if (chosenClientId) {
                    loadPicsForClient(chosenClientId, preselectedPicId || initialPicId);
                }
            })
            .catch(() => {
    ...
    function loadPicsForClient(clientId, selectedPicId = null) {
        ...
        fetch(`/smart-quote/pics/${clientId}`)
            ...
            .then(data => {
                ...
                if (picList.length > 0) {
                    picList.forEach(p => {
                        const isSel = (selectedPicId && selectedPicId == p.id) || (initialPicId && initialPicId == p.id) ? 'selected' : '';
            ...
            <tr id="item_row_${sIndex}_${iIndex}" data-section="${sIndex}" data-item="${iIndex}">
                ...
                <td>
                    ...
                    ${itemType === 'material' ? `
                        ...
                        <input type="text" name="sections[${sIndex}][items][${iIndex}][item_name]" class="form-control-clean form-control-sm w-100 item-name-input" value="${itemName}" placeholder="Deskripsi/keterangan...">
                    ` : `
                        <input type="text" name="sections[${sIndex}][items][${iIndex}][item_name]" class="form-control-clean form-control-sm w-100 item-name-input fw-semibold mb-1" value="${itemName}" placeholder="Nama pekerjaan / alat..." required>
                        <input type="text" name="sections[${sIndex}][items][${iIndex}][spec]" class="form-control-clean form-control-sm w-100" value="${spec || ''}" placeholder="Spesifikasi / detail tambahan...">
                    `}
