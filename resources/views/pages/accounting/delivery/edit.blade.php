...
@section('title', 'Edit Surat Jalan ' . $delivery->do_number)

@section('content')
    <div class="d-flex justify-content-between align-items-center py-3 mb-3">
        <div>
            <h4 class="fw-bold mb-1">
                <span class="text-muted fw-light">Accounting / <a href="{{ route('delivery.index') }}" class="text-muted">Delivery Order</a> /</span> Edit Surat Jalan
            </h4>
            <p class="text-muted mb-0 small">Perbarui data Surat Jalan / Delivery Order <strong>#{{ $delivery->do_number }}</strong></p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('delivery.show', $delivery->id) }}" class="btn btn-outline-secondary d-flex align-items-center gap-1 shadow-xs">
                <i class="mdi mdi-arrow-left me-1"></i> Kembali ke Detail DO
            </a>
        </div>
    </div>
    ...
    <form id="formEditManualDelivery" action="{{ route('delivery.update', $delivery->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body p-4">
                @php
                    $sourceType = old('source_type', $delivery->id_invoice ? 'invoice' : 'standalone');
                    $deliveryDateVal = old('date', $delivery->date ? \Carbon\Carbon::parse($delivery->date)->format('Y-m-d') : '');
                @endphp
                        ...
                        <div class="d-flex flex-wrap gap-3">
                            <div class="form-check custom-option custom-option-basic">
                                <label class="form-check-label custom-option-content py-2 px-3 rounded border bg-white cursor-pointer" for="sourceInvoice">
                                    <input name="source_type" class="form-check-input" type="radio" value="invoice" id="sourceInvoice" {{ $sourceType === 'invoice' ? 'checked' : '' }}>
                            ...
                            <div class="form-check custom-option custom-option-basic">
                                <label class="form-check-label custom-option-content py-2 px-3 rounded border bg-white cursor-pointer" for="sourceStandalone">
                                    <input name="source_type" class="form-check-input" type="radio" value="standalone" id="sourceStandalone" {{ $sourceType === 'standalone' ? 'checked' : '' }}>
                                    <span class="custom-option-header pb-0">
                ...
                <div class="row g-3">
                        ...
                        <select class="form-select select2" id="selectInvoiceDo" name="id_invoice" style="width: 100%;">
                            ...
                            @foreach ($invoices as $inv)
                                @php
                                    ...
                                    $selected = old('id_invoice', $delivery->id_invoice) == $inv->id;
                                @endphp
                                <option value="{{ $inv->id }}"
                                    {{ $selected ? 'selected' : '' }}
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="doEntity" name="entity" required>
                            <option value="Reftech" {{ old('entity', $delivery->entity_display) === 'Reftech' ? 'selected' : '' }}>PT Reftech Jaya Optima</option>
                            <option value="Kojisha" {{ old('entity', $delivery->entity_display) === 'Kojisha' ? 'selected' : '' }}>PT Kojisha Innotiv Indonesia</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="doType" name="type" required>
                            <option value="ekspedisi" {{ old('type', strtolower($delivery->type ?? 'ekspedisi')) === 'ekspedisi' ? 'selected' : '' }}>Ekspedisi / Logistik / Kurir</option>
                            <option value="teknisi" {{ old('type', strtolower($delivery->type ?? '')) === 'teknisi' ? 'selected' : '' }}>Teknisi / Hand Carry / Diantar Sendiri</option>
                        </select>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label fw-bold">Tanggal Surat Jalan <small class="text-muted fw-normal">(Opsional / Boleh Kosong)</small></label>
                        <input type="date" class="form-control" id="doDate" name="date" value="{{ $deliveryDateVal }}">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="doCustomerName" name="customer_name" value="{{ old('customer_name', $delivery->customer_name_display) }}" placeholder="Masukkan nama PT / Perusahaan / Customer" required>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label fw-bold">No. PO Customer / Referensi</label>
                        <input type="text" class="form-control" id="doPoNumber" name="po_number" value="{{ old('po_number', $delivery->po_number_display) }}" placeholder="Contoh: PO/2026/09/001">
                    </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            ...
                            <div id="addressSwitcher" class="d-none">
                                <div class="btn-group btn-group-sm" role="group">
                                    <input type="radio" class="btn-check" name="dest_toggle" id="destMain" value="1" {{ old('destination', $delivery->destination ?? '1') == '1' ? 'checked' : '' }} autocomplete="off">
                                    <label class="btn btn-outline-primary btn-xs" for="destMain">Alamat Utama</label>

                                    <input type="radio" class="btn-check" name="dest_toggle" id="destSub" value="2" {{ old('destination', $delivery->destination ?? '1') == '2' ? 'checked' : '' }} autocomplete="off">
                                    <label class="btn btn-outline-primary btn-xs" for="destSub">Alamat Cabang/Pabrik</label>
                                </div>
                                <input type="hidden" name="destination" id="doDestination" value="{{ old('destination', $delivery->destination ?? '1') }}">
                            </div>
                        </div>
                        <textarea class="form-control" id="doAddress" name="address" rows="2" placeholder="Masukkan alamat lengkap tujuan pengiriman" required>{{ old('address', $delivery->address_display) }}</textarea>
                    </div>

                    {{-- Row 5: Info Tambahan Pengiriman --}}
                    <div class="col-md-4">
                        <label class="form-label fw-semibold">No. Surat Jalan</label>
                        <input type="text" class="form-control font-monospace" id="doNoDo" name="no_do" value="{{ old('no_do', $delivery->do_number) }}" placeholder="Otomatis jika dikosongkan">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" id="doDriverName" name="driver_name" value="{{ old('driver_name', $delivery->driver_name) }}" placeholder="Misal: JNE / Pak Budi / GoSend">
                    </div>

                    <div class="col-md-4">
                        <label class="form-label fw-semibold">No. Plat Kendaraan / No. Resi</label>
                        <input type="text" class="form-control" id="doVehicleNo" name="vehicle_no" value="{{ old('vehicle_no', $delivery->vehicle_no) }}" placeholder="Misal: B 1234 XYZ / Resi #12345">
                    </div>

                    <div class="col-md-12">
                        <label class="form-label fw-semibold">Catatan Pengiriman</label>
                        <textarea class="form-control" id="doNote" name="note" rows="2" placeholder="Catatan khusus penerima / instruksi bongkar muat...">{{ old('note', $delivery->note) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="table-responsive border rounded mb-3">
                    <table class="table table-sm table-bordered align-middle mb-0" id="doItemsTable">
                        ...
                        <tbody id="doItemsTableBody">
                            {{-- Rows populated via Blade and JS --}}
                        </tbody>
                    </table>
                </div>
                <div class="form-text text-muted small">
                    <i class="mdi mdi-information-outline me-1"></i>Pastikan minimal 1 baris item barang terisi dengan nama barang dan kuantitas.
                </div>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm mb-5">
            <div class="card-body py-3 d-flex justify-content-between align-items-center">
                <a href="{{ route('delivery.show', $delivery->id) }}" class="btn btn-label-secondary">
                    <i class="mdi mdi-close me-1"></i> Batal
                </a>
                <button type="submit" class="btn btn-primary d-flex align-items-center gap-1 shadow-sm px-4" id="btnSubmitDoEdit">
                    <i class="mdi mdi-content-save-check-outline fs-5"></i> Simpan Perubahan Surat Jalan
                </button>
            </div>
        </div>
    </form>
...
$(document).ready(function() {
    ...
    const existingItems = @json($dDelivery ?? []);
    ...
    function createDoItemRow(idx, data = {}) {
        const productVal = escapeHtml(data.product || '');
        const qtyVal     = data.qty !== undefined && data.qty !== null ? data.qty : '1';
        const unitVal    = escapeHtml(data.info_qty || 'Pcs');

        return `
            <tr data-row-idx="${idx}">
                <td class="text-center fw-bold text-muted row-number" style="vertical-align: middle;">${idx + 1}</td>
                <td>
                    <input type="text" class="form-control form-control-sm" name="items[${idx}][product]" value="${productVal}" placeholder="Nama barang / part" required>
                </td>
                <td>
                    <input type="number" step="any" min="0.01" class="form-control form-control-sm text-center fw-bold" name="items[${idx}][qty]" value="${qtyVal}" required>
                </td>
                <td>
                    <input type="text" class="form-control form-control-sm text-center" name="items[${idx}][info_qty]" value="${unitVal}" placeholder="Pcs / Unit / Set" required>
                </td>
                <td class="text-center" style="vertical-align: middle;">
                    <button type="button" class="btn btn-xs btn-label-danger btn-icon btn-remove-row" title="Hapus baris ini">
    ...
    function reindexDoRows() {
        $('#doItemsTableBody tr').each(function(i) {
            ...
            $(this).attr('data-row-idx', i);
            $(this).find('input[name*="[product]"]').attr('name', `items[${i}][product]`);
            $(this).find('input[name*="[qty]"]').attr('name', `items[${i}][qty]`);
            $(this).find('input[name*="[info_qty]"]').attr('name', `items[${i}][info_qty]`);
        });
    }
    ...
    function initRows() {
        if (existingItems && existingItems.length > 0) {
            existingItems.forEach((item, idx) => {
                $('#doItemsTableBody').append(createDoItemRow(idx, item));
            });
            doRowIndex = existingItems.length;
        } else {
