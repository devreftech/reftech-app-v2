{{--
    Blok Note + Ketentuan Rental + Term & Condition — dipakai di detail.blade.php,
...
<div style="border:1px solid #e0e0e0; border-radius:6px; padding:12px 16px; font-size:12px; background:#fff; margin-bottom:16px;">
    ...
    <table style="width:100%; border-collapse:collapse;">
        <tr>
            ...
            <td style="padding:3px 0; vertical-align:top;">: {{ $tcValidity ?? '-' }}</td>
        </tr>
        <tr>
            <td style="padding:3px 0; color:#555; vertical-align:top;">Price</td>
            <td style="padding:3px 0; vertical-align:top;">: {{ $tcPricing ?? '-' }}</td>
        </tr>
        <tr>
            <td style="padding:3px 0; color:#555; vertical-align:top;">Delivery Process</td>
            <td style="padding:3px 0; vertical-align:top; white-space:pre-line;">: {{ $tcDeliveryProcess ?? '-' }}</td>
        </tr>
        <tr>
            <td style="padding:3px 0; color:#555; vertical-align:top;">Payment</td>
            <td style="padding:3px 0; vertical-align:top;">: {{ $tcPayment ?? '-' }}</td>
        </tr>
        @if (!empty($tcWarranty))
        <tr>
            <td style="padding:3px 0; color:#555; vertical-align:top;">Warranty</td>
            <td style="padding:3px 0; vertical-align:top;">: {{ $tcWarranty }}</td>
        </tr>
        @endif
    </table>
</div>
