    ...
    <div class="container-fluid flex-grow-1 container-p-y p-0 goods-receipt-page">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div>
                ...
                <p class="text-muted mb-0 small">Verifikasi kesesuaian fisik barang yang diterima dari supplier — PO dibeli langsung tanpa Purchase Request.</p>
            </div>
            <div class="d-flex align-items-center gap-2 flex-wrap">
                ...
                <a href="{{ route('purchase.show', $po->id) }}" class="btn btn-outline-secondary btn-sm">
        ...
        {{-- Form action --}}
        <form action="{{ route('purchase.store-goods-receipt-direct', $po->id) }}" method="POST">
            @csrf
            ...
            <div class="card modern-card mb-0">
                            ...
                            <tbody>
                                @foreach ($details as $key => $detail)
                                    ...
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>
                                            <span class="fw-semibold text-dark">{{ $detail->product }}</span>
                                            <small class="text-muted d-block" style="font-size: 11px;">PO No: {{ $po->no_po }}</small>
                                        </td>
                                        <td>
                                            <span class="fw-bold text-dark fs-6" id="qty-order-{{ $key }}">{{ $detail->qty }}</span>
                                            <span class="text-muted small d-block">{{ $detail->info_qty ?: 'pcs' }}</span>
                                        </td>
                                        <td>
                                            ...
                                            <select class="form-select form-select-sm" name="replacement[{{ $key }}]" required style="min-width: 200px;">
                                                ...
                                                @foreach ($fullRep[$key] as $products)
                                                    <option value="{{ $products->id }}" {{ count($fullRep[$key]) == 1 ? 'selected' : '' }}>
                                                        {{ $products->product->commodity ?? $detail->product }} ({{ $products->replacement }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        </td>
                ...
                <div class="card-footer bg-white border-top p-3 d-flex justify-content-end gap-2">
                    <a href="{{ route('purchase.show', $po->id) }}" class="btn btn-outline-secondary">
        ...
        $(document).ready(function() {
            ...
            $('.gr-status-radio').on('change', function() {
                ...
                if (val === 'Sesuai') {
                    ...
                    $qtyInput.css('border-color', '#d9ade8');
                    ...
                    $qtyInput.css('border-color', '#ff9f43');
                    $qtyInput.focus();
                    ...
                    $qtyInput.prop('readonly', true);
                    $qtyInput.css('background-color', '#f1f5f9');
                    $qtyInput.css('border-color', '#ff4d49');
