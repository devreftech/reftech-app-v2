<form action="{{ route('insert_fee_service.quotation', $quote->id) }}" method="post" enctype="multipart/form-data">
    @csrf
                            ...
                            <div class="row">
                                ...
                                @foreach ($subQuote as $subtitle)
                                    ...
                                    @foreach ($subtitle->detail as $item)
                                        ...
                                        @endphp
                                        <div class="col-4">
                                            <p class="text-start">Fee {{ chr($abjad) }}-{{ $no }}</p>
                                        </div>
                                ...
                                <div class="col-8 mb-3">
                                    <div class="form-floating-outline">
                                        <input type="text" id="totalLabel"
                                            ...
                                            value="Rp {{ old('total[]', $quote->fee != 0 ? number_format($quote->fee, 0, ',', '.') : 0) }}"
                                            disabled>
                                        <input class="form-control invoice-item-total" type="number" name="total"
                                            id="total"
                                            value="{{ old('total[]', $quote->fee != 0 ? $quote->fee : 0) }}" hidden>
                                    </div>
                                </div>
                            </div>
                        </form>
