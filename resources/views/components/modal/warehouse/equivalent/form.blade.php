        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="exampleModalLabel5-{{ $uid }}"> Create Equivalent
                    </h4>
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col mb-2">
                            <div class="form-floating form-floating-outline">
                                <input type="text" id="image-{{ $uid }}" class="form-control" name="image"
                                    placeholder=" Example : https://drive.google.com/drive/folders/**********" value="{{ old('image', @$serial->image ?? '') }}">
                                <label for="image-{{ $uid }}">Image ( Link GDrive )</label>
                            </div>
                        </div>
                        <div class="col mb-2">
                            <div class="form-floating form-floating-outline">
                                <input type="text" id="brand-{{ $uid }}" class="form-control" name="brand"
                                    placeholder="xxx x xxx x xxx x" value="{{ old('brand', @$serial->brand ?? '') }}">
                                <label for="brand-{{ $uid }}">Brand</label>
                            </div>
                        </div>
                    </div>
                    <div class="row g-2 mb-3">
                        <div class="col mb-2">
                            <div class="form-floating form-floating-outline">
                                <input type="text" id="pn-{{ $uid }}" class="form-control" name="pn"
                                    placeholder="xxxx@xxx.xx" value="{{ old('pn', @$serial->pn ?? '') }}">
                                <label for="pn-{{ $uid }}">Part Number</label>
                            </div>
                        </div>
                        <div class="col mb-2">
                            <div class="input-group form-floating form-floating-outline" data-price="1">
                                <span class="input-group-text">Rp. </span>
                                <input type="text" class="form-control invoice-item-price-label" id="price-label-{{ $uid }}"
                    ...
                    <input type="text" id="detail-{{ $uid }}" class="form-control" name="detail"
