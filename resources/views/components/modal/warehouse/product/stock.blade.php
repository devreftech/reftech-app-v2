                            ...
                            <div class="row g-3">
                                <div class="col-md-7">
                                    <div class="form-floating form-floating-outline">
                                        <input type="number" step="any" id="first_stock_{{ $product->id }}" class="form-control bg-white"
                                            name="first_stock" value="{{ old('first_stock', $product->first_stock ?? 0) }}"
                                            placeholder="Stok Awal">
                                        <label for="first_stock_{{ $product->id }}">First Stock ({{ $product->unit ?? 'Pcs' }})</label>
                                    </div>
                                </div>
                                <div class="col-md-5">
                                    <div class="form-floating form-floating-outline">
                                        <input type="date" id="date_{{ $product->id }}" class="form-control bg-white" name="date"
                                            value="{{ old('date', $product->date ?? now()->format('Y-m-d')) }}">
                                        <label for="date_{{ $product->id }}">Tanggal Berlaku</label>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row g-2">
                            ...
                            <div class="col-md-4 col-sm-6">
                                <div class="card border h-100 p-3 rounded-3 shadow-none position-relative overflow-hidden"
                                    ...
                                    @if (count($details) > 0)
                                        <input type="hidden" id="recent-warehouse-stock-{{ $product->id }}"
                                        ...
                                        <div class="d-flex align-items-baseline gap-1">
                                            ...
                                            <small class="text-muted fw-semibold" style="font-size: 12px;">{{ $product->unit ?? 'Pcs' }}</small>
                                        </div>
                                    @else
                                        <div class="input-group input-group-sm">
                                            <input type="number" step="any" id="recent-warehouse-stock-{{ $product->id }}"
                                            ...
                                            <span class="input-group-text bg-light text-muted">{{ $product->unit ?? 'Pcs' }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="col-md-4 col-sm-6">
                                <div class="card border h-100 p-3 rounded-3 shadow-none position-relative overflow-hidden"
                                    ...
                                    @if (count($details) > 0)
                                        <input type="hidden" id="recent-office-stock-{{ $product->id }}"
                                    ...
                                    @else
                                        <div class="input-group input-group-sm">
                                            <input type="number" step="any" id="recent-office-stock-{{ $product->id }}"
                            ...
                            <div class="col-md-4 col-sm-12">
                                <div class="card border h-100 p-3 rounded-3 shadow-none position-relative overflow-hidden"
                                    style="background-color: #ffffff; border-color: #e2e8f0 !important;">
                                    <div class="d-flex align-items-center justify-content-between mb-2">
                                        <span class="text-secondary fw-semibold text-uppercase" style="font-size: 11px; letter-spacing: 0.5px;">Pending Stock</span>
                                        <div class="avatar avatar-xs rounded-circle d-flex align-items-center justify-content-center"
                                            style="width: 28px; height: 28px; background: #fef3c7; color: #b45309;">
                                            <i class="mdi mdi-timer-sand fs-6"></i>
                                        </div>
                                    </div>
                                    <div class="input-group input-group-sm">
                                        <input type="number" step="any" id="pending_stock_{{ $product->id }}"
                                            class="form-control text-center fw-bold pending-stock-input"
                                            style="color: #b45309; background: #fffdf5; border-color: #fde68a;"
                                            name="pending_recent_stock"
                                            value="{{ old('pending_recent_stock', $product->pending_stock ?? 0) }}">
                                        <span class="input-group-text text-muted" style="background: #fef3c7; border-color: #fde68a;">{{ $product->unit ?? 'Pcs' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex align-items-center justify-content-between p-3 mt-3 rounded-3 border"
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <small class="text-muted" style="font-size: 11px;">Warehouse + Office + Pending</small>
                                </div>
                            </div>
                            <span class="badge rounded-pill px-3 py-2 fw-bold fs-6 total-all-stock-preview"
                                ...
                                {{ ($product->stock ?? 0) + ($product->warehouse_stock ?? 0) + ($product->pending_stock ?? 0) }} {{ $product->unit ?? 'Pcs' }}
                            </span>
                        </div>
                    </div>
                                        ...
                                        @foreach ($details as $index => $detail)
                                            <tr class="replacement-row">
                                                ...
                                                <td>
                                                    <div class="input-group input-group-sm">
                                                        <input type="number" step="any"
                                                            id="office-stock-{{ $product->id }}-{{ $index }}"
                                                ...
                                                <td>
                                                    <div class="input-group input-group-sm">
                                                        <input type="number" step="any"
                                                            id="warehouse-stock-{{ $product->id }}-{{ $index }}"
                        ...
                        @else
                            <div class="text-center p-4 border rounded-3 bg-light">
                                ...
                                <p class="text-muted mb-0" style="font-size: 12px;">
                                    Produk ini tidak memiliki daftar part replacement. Anda dapat menginput stok Office & Warehouse langsung pada kotak ringkasan di atas.
                                </p>
                            </div>
                        @endif
                    </div>
...
@push('script')
    <script>
        $(() => {
            ...
            const unitName = '{{ $product->unit ?? "Pcs" }}';
            ...
            function calculateTotals() {
                ...
                const pendingVal = parseFloat($modal.find('.pending-stock-input').val()) || 0;
                const grandTotal = totalOffice + totalWarehouse + pendingVal;

                $modal.find('.total-all-stock-preview').text(grandTotal + ' ' + unitName);
            }

            $(modalId).on('input keyup change', '.office-stock, .warehouse-stock, .pending-stock-input, .manual-stock-input', function() {
                calculateTotals();
            });
